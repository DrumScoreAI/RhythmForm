...
# Randomize start position to reduce contention when running many parallel instances
range=$((xvfb_display_max - xvfb_display_min))
start_offset=$((RANDOM % range))
count=0

while (( count < range )); do
  # Calculate current display number candidates
  i=$((xvfb_display_min + (start_offset + count) % range))
