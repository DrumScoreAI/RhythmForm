...
def _parse_smt_token(token_str):
    ...
    if token_type in ["title", "subtitle", "composer", "creator", "clef", "time"]:
        ...
        # content is like "3:start" or "3:stop"
        token_info["value"] = content
...
def smt_to_musicxml(smt_string: str) -> music21.stream.Score:
    ...
    This version creates a flat stream of musical events and then uses 
    music21's makeMeasures() to automatically create well-formed measures.
    This robustly handles SMT generated by models which may not have perfect
    measure durations, preventing MusicXML corruption.
    """
    try:
        ...
        time_signature_obj = None # Keep track of the time signature
        ...
        for token_str in header_tokens:
            ...
            if token["type"] == "title":
            ...
            elif token["type"] in ["creator", "composer"]:
                if not score.metadata:
                    score.metadata = music21.metadata.Metadata()
                ...
                time_signature_obj = music21.meter.TimeSignature(token["value"])
                part.insert(0, time_signature_obj)

        # --- Process body into a flat stream ---
        body_tokens = body_string.replace('|', ' ').strip().split()
        tuplet_state = None

        for token_str in body_tokens:
            token = _parse_smt_token(token_str)
            if not token:
                continue
            ...
            if token["type"] == "text":
                element_to_add = music21.expressions.TextExpression(token["value"])

            elif token["type"] == "tuplet":
                value = token.get("value", "")
                if ":start" in value:
                    num_notes_str = value.split(':')[0]
                    try:
                        num_notes = int(num_notes_str)
                        # SMT is lossy, assume common tuplet ratios (e.g., 3 in time of 2)
                        normal_notes = 2 if num_notes == 3 else None
                        tuplet_state = music21.duration.Tuplet(numberNotesActual=num_notes, numberNotesNormal=normal_notes)
                    except (ValueError, TypeError):
                        tuplet_state = None
                elif ":stop" in value:
                    tuplet_state = None
                continue # Tuplet tokens don't create musical elements themselves
                ...
                if len(token["instruments"]) > 1:
                    ...
                    for instrument_name in token["instruments"]:
                        if instrument_name in SMT_TO_DRUM_DISPLAY:
                            ...
                            n.displayStep = display_info['display_step']
                            n.displayOctave = display_info['display_octave']
                    ...
                    if any(isinstance(n, music21.note.Unpitched) for n in chord_notes):
            ...
            if element_to_add:
                part.append(element_to_add)

        # Automatically create measures from the flat stream of notes.
        # This is the key fix for preventing corrupt files due to incorrect measure durations.
        if time_signature_obj:
            part.makeMeasures(inPlace=True)
        else:
            part.makeMeasures(inPlace=True) # Use default 4/4
