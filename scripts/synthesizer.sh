...
clean_training_data() {
    ...
    find "$TRAINING_DATA_DIR" -path "$TRAINING_DATA_DIR/fine_tuning" -prune -o -path "$TRAINING_DATA_DIR/*/*" -type f \( -name "*.xml" -o -name "*.pdf" -o -name "*.png" -o -name "*.csv" -o -name "*.json" -o -name "*.smt" \) -exec rm -vf {} +
...
if ! xvfb-run -a bash -c "cat $temp3 | xargs -P $pdf_cores -I {} bash -c '$RHYTHMFORMHOME/scripts/mscore_convert.sh -i \"\$0\" -f pdf &> >(grep -v \"Invalid QML element name\" -v \"pw.con\" >&2)' {}"; then
